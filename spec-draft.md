...
| **Trả lời** | *Người dùng đang di chuyển (đi bộ, mang đồ, vội). Pain: phải mở app, nhập địa chỉ mất 30–60s. AI: nhận voice → hiểu intent → auto-fill điểm đi/đến, loại phương tiện → giảm xuống \<5s.* | *Sai điểm đến \= cost cao (tiền \+ thời gian). → Luôn hiển thị map preview \+ địa chỉ trước khi "Accept". User sửa bằng: edit text hoặc nói lại.* | *Latency \<2s (speech-to-text \+ NLU \+ fetch data). Cost thấp (voice \+ LLM \+ API). Risk: noise → nhận sai, ambiguity ("quán phở"), location mismatch.* |
